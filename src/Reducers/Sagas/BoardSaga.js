...
export const getBoardsSaga = () => {
  return { type: GET_BOARDS };
};
...
export const createBoardsSaga = ({ name, id }) => {
  return { type: CREATE_BOARDS, name, id };
};
function* watchCreateBoards() {
  yield takeEvery(CREATE_BOARDS, function* ({ name, id }) {
    ...
    if (!result.status) {
      ...
      let listsId = yield select((state) =>
        state.lists.data.filter((el) => el.boardId === id).map((el) => el.id)
      );
...
export const deleteBoardsSaga = ({ boardId }) => {
  return { type: DELETE_BOARDS, boardId };
};
function* watchDeleteBoards() {
  yield takeEvery(DELETE_BOARDS, function* ({ boardId }) {
    ...
    if (!result.status) {
      ...
      let listsId = yield select((state) =>
        state.lists.data.filter((el) => el.boardId === boardId).map((el) => el.id)
      );
